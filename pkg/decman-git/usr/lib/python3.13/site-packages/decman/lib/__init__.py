    ...
    def install(self, packages: list[str]):
        ...
        if packages:
            try:
                ...
                # Don't fail completely if this fails - just warn
                print_warning(
                    f"Failed to mark some packages as explicitly installed: {error}"
                )
                print_debug(f"Packages that failed: {packages}")
    ...
    def install_files(self, files: list[str], as_explicit: list[str]):
        ...
        if as_explicit:
            try:
                ...
                print_debug(f"Packages that failed: {as_explicit}")
